...
def weighted_sum(x: torch.Tensor, weight: torch.Tensor):
    ...
    return einsum(x, weight, "... D, D -> ...").sum(dim=-1)
...
def weighted_sum_backward(
    x_ptr, weight_ptr,                 # Inputs
    grad_output_ptr,                   # Grad input (dL/dy)
    grad_x_ptr, partial_grad_weight_ptr,  # Grad outputs (dL/dx, partial dL/dw per row-tile)
    stride_xr, stride_xd,
    stride_wd,
    stride_gr,
    ...
    # Identify which tile of rows this program instance handles
    row_tile_idx = tl.program_id(0)
    n_row_tiles = tl.num_programs(0)

    # Block pointer for grad_output: shape (NUM_ROWS,)
    ...
    # Block pointer for x: shape (NUM_ROWS, D)
    x_block_ptr = tl.make_block_ptr(
        x_ptr,
        shape=(NUM_ROWS, D),
    ...
    # Block pointer for weight: shape (D,)
    weight_block_ptr = tl.make_block_ptr(
        weight_ptr,
        shape=(D,),
        strides=(stride_wd,),
    ...
    # Block pointer for grad_x: shape (NUM_ROWS, D)
    ...
    # Block pointer for partial grad_weight: shape (n_row_tiles, D)
    # Each program writes a single-row partial gradient for its row tile
    ...
    # Load grad_output for this row tile: shape (ROWS_TILE_SIZE,)
    grad_output = tl.load(
    ...
    for _ in range(tl.cdiv(D, D_TILE_SIZE)):
        # Load current weight tile: shape (D_TILE_SIZE,)
        weight = tl.load(
            weight_block_ptr,
            boundary_check=(0,),
            padding_option="zero",
        ...
        # Compute grad_x tile via outer product: (ROWS_TILE_SIZE, D_TILE_SIZE)
        ...
        # Load current x tile for partial grad_weight reduction: (ROWS_TILE_SIZE, D_TILE_SIZE)
        row = tl.load(
            x_block_ptr,
            boundary_check=(0, 1),
            padding_option="zero",
        ...
        # Reduce over rows to produce a 1xD_TILE_SIZE partial gradient for weight
        # grad_weight_tile = sum_i x[i, j] * grad_output[i]
        grad_weight_row = tl.sum(row * grad_output[:, None], axis=0, keep_dims=True)

        # Store partial grad_weight for this tile; dim 0 is always in-bounds (size 1)
        tl.store(
        ...
        # Advance block pointers to the next D tile
...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, weight: torch.Tensor):
        ...
        assert ctx.D_TILE_SIZE != 0, "invalid d tile size, triton.next_power_of_2(D) should be more than 16"
        # Each thread processes 16 batch elements at a time
        ctx.ROWS_TILE_SIZE = 16
        ctx.input_shape = input_shape

        # Need to initialize empty result tensor.
        # Note that these elements are not necessarily 0!
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        # 每个 thread block 先写入 partial buffer，再 reduce 得到最终 grad_weight
...
if __name__ == "__main__":
    D = 16
    x = torch.ones((1,D), requires_grad=True).cuda()
